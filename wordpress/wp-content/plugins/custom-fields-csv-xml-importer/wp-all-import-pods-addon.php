...
Plugin URI: https://vipp.com.au/cio-custom-fields-importer/product/cio-pro/
Description: Efficiently import large volume of csv or xml data into custom fields created with pods. The premium version allows you to import to any custom fields created by other plugins.  This plugin needs both WP All Import and Pods plugins to import, both freely available. <a href="https://vipp.com.au/cio-custom-fields-importer/product/cio-pro/"> Upgrade to Professional Version </a> for auto media upload, auto matching of relationship fields, user import, WooCommerce customer import, import of pods advanced content types, and more features.
Version: 1.0.3
License: GPLv2
Author: <a href="https://vipp.com.au/cio-custom-fields-importer/">VisualData</a>
*/
...
if ($my_pods) {
	...
	foreach ($my_pods as $my_pod) {
		...
		if ('post_type'==get_post_meta($my_pod->ID,'type',true)) {
			//keep track of enabled content types to invoke the custom import function
			$my_pod_array[]=$my_pod->post_name;
			
			//track custom fields to save in custom import function
			...
			if ($pods_fields) {
				...
				foreach ($pods_fields as $pods_field) {
					//organize custom fields by content type, so users may drag to the fields under the target type
			...
			//display custom fields by enabled content type
